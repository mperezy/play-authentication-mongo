...
import jp.t2v.lab.play2.auth.AuthElement
import models.Role.{Normal, Admin}
import play.modules.reactivemongo.json.collection.JSONCollection
...
object AccountsCtrl extends Controller with MongoController  with AuthElement with AuthConfigImpl {
  ...
  def byAtelier(id: String) =  AsyncStack(AuthorityKey -> Admin, AuthorityKey -> Normal) {
    implicit request => {
      val cursor: Cursor[Account] = collection.
  ...
  def byId(id: String) =  AsyncStack(AuthorityKey -> Admin, AuthorityKey -> Normal) {
    implicit request => {
      val cursor: Cursor[Account] = collection.
  ...
  def createUser =  AsyncStack(parse.json){
    implicit request => request.body.validate[Account].map {
      profile: Account => {
  ...
  def uploadAvatar(user_id: String) = AsyncStack(parse.multipartFormData, AuthorityKey -> Admin, AuthorityKey -> Normal) {
